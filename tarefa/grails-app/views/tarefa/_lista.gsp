    ...
    <g:each var="tarefa" in="${tarefas}" status="i">
      <tbody>
        <tr class="${((i % 2) == 0 ? 'even' : 'odd')}
          ...
          <td class="${tarefa.completada ? 'taskCompleted': ''}">${tarefa.deadline.format('dd/MM/yyyy')}</td>
          ...
          <td>
            ...
            <g:if test="${!tarefa.completada}">
                  ...
                  <g:remoteLink controller="tarefa" action="alterar" update="frmTarefas" id="${tarefa.id}" class="editRow">Editar</g:remoteLink>
                  &nbsp;
                  <g:remoteLink controller="tarefa" action="completar" update="completa" id="${tarefa.id}" class="completeRow" onSuccess="carregarLista()">Completar</g:remoteLink>
                  &nbsp;
            </g:if>
...
<g:else>
  <h3> Não tem tarefas inseridas!</h3>
</g:else>
<br/>